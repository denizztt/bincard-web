...
import { BUS_VALIDATION } from '../constants/busTypes';
...
const BusEdit = () => {
  ...
  const loadBusData = async () => {
    try {
      ...
      setError('Otobüs bilgileri yüklenirken hata oluştu: ' + (error.message || 'Bilinmeyen hata'));
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Updating bus with data:', submitData);
      const response = await busApi.updateBus(parseInt(id), submitData);
      ...
      if (response && isSuccess) {
        setSuccess('Otobüs başarıyla güncellendi!');
        setTimeout(() => {
          navigate(`/bus/${id}`);
        }, 2000);
      } else {
        setError(response?.message || 'Otobüs güncellenirken hata oluştu');
      }
    } catch (err) {
      console.error('Error updating bus:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Otobüs güncellenirken hata oluştu';
    ...
    return (
      <div style={{ padding: '20px', textAlign: 'center', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div>
          ...
          <p>Otobüs bilgileri yükleniyor...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bus-add-container">
      ...
      <div className="page-header">
        <div className="header-left">
          ...
          <div>
            <h1><Bus size={24} /> Otobüs Düzenle</h1>
            <p>Otobüs bilgilerini güncelleyin</p>
          </div>
        </div>
      </div>
